    ...
    public static <T> TableInfo getTableInfo(T domain) {
        ...
        for (Field field : fields) {
            ...
            if (null != ecolumn) {
                ...
                boolean isPrimaryKey = DEFAULT_PRIMATRY_KEY_NAME.equals(field.getName());
                if (isPrimaryKey) {
                    table.getAllPrimaryKeyFields().add(field);
                }
                ...
                List<Column> columnList = isPrimaryKey ? table.getPrimaryKeyColumns() : table.getCommonColumns();
                if (null != value) {
                    columnList.add(new Column(field.getName(), value));
                    table.getAllColumns().add(new Column(field.getName(), value));
                }
